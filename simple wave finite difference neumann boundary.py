...
# u[1, nx] = c_1 * (2 * dxdt * end_forcing_slope(1) + u[0, nx - 1]) + c_2 * u[0, nx] + c_1 * u[0, nx - 1] - dt * vi(x[nx])
...
for i in range(1, nt):
    ...
    # u[i + 1, nx] = c_1 * (2 * dxdt * end_forcing_slope(i) + u[i, nx - 1]) + c_2 * u[i, nx] + c_1 * u[i, nx - 1] - u[i - 1, nx]

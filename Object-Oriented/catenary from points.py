...
class Point:
    ...
    def update(self):
        ...
        left_distance = sqrt((self.x - self.left.x) ** 2 + (self.y - self.left.y) ** 2) - self.left_initial_distance
        right_distance = sqrt((self.x - self.right.x) ** 2 + (self.y - self.right.y) ** 2) - self.right_initial_distance
        ...
        length_of_segment = (left_distance / 2 + right_distance / 2)
        length_of_segment_x = left_distance * 0.5 * cos(a_left) + right_distance * 0.5 * cos(a_right)
        length_of_segment_y = left_distance * 0.5 * sin(a_left) + right_distance * 0.5 * sin(a_right)
        # update the forces towards the left and right neighbors based on hook's law
        # F = k * displacement
        f_left = s_c_corrected * left_distance
        f_right = s_c_corrected * right_distance
        ...
        net_force_x = f_right * cos(a_right) + f_left * cos(a_left) - drag_force_x
        ...
        net_force_y = f_left * sin(a_left) + f_right * sin(a_right) + gravitational_force - drag_force_y

        # acceleration = force/mass
        x_accel = net_force_x/self.mass
        y_accel = net_force_y/self.mass
...
'''
spring_constant = 800     # units: Newtons per meter
gravitational_acceleration = -9.81   # units: meters per second squared (negative is downwards)
linear_density = 15      # units: kilograms per meter

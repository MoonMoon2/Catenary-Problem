...
class Point:
    ...
    def update(self):
        ...
        self.left_distance = sqrt((self.x - self.left.x) ** 2 + (self.y - self.left.y) ** 2)*0.5 - 0.9*self.left_initial_distance
        self.right_distance = sqrt((self.x - self.right.x) ** 2 + (self.y - self.right.y) ** 2)*0.5 - 0.9*self.right_initial_distance
        ...
        # F = k * displacement
        spring_f_left = s_c_corrected * self.left_distance
        spring_f_right = s_c_corrected * self.right_distance
        ...
        self.net_force_x = (spring_f_right + f_right) * cos(a_right) + (spring_f_left + f_left) * cos(
            a_left) - drag_force_x + f_left * cos(a_left) + f_right * cos(a_right)
...
'''
spring_constant = 2500     # units: Newtons per meter
gravitational_acceleration = -9.81   # units: meters per second squared (negative is downwards)
linear_density = 1.4      # units: kilograms per meter
...
base_tension = 10 * 0.001**2

t0 = 0  # initial time (seconds)
tf = 120  # final time (seconds)
